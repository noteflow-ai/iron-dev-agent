...
echo "===== PRD & UI 生成器热部署启动脚本 ====="
echo "正在检查是否有服务器实例正在运行..."

# 强制停止所有可能占用3000端口的进程
force_stop_port() {
    echo "尝试强制停止所有占用3000端口的进程..."
    ...
    # 如果lsof不可用，尝试使用netstat
        ...
        PIDS=$(netstat -nlp 2>/dev/null | grep ':3000 ' | awk '{print $7}' | cut -d'/' -f1 || echo "")
        if [ ! -z "$PIDS" ]; then
            echo "发现以下进程占用端口3000: $PIDS"
            for PID in $PIDS; do
                echo "正在强制终止进程 $PID..."
    ...
    # 如果netstat也不可用，尝试使用ss
        ...
        PIDS=$(ss -nlp 2>/dev/null | grep ':3000 ' | awk '{print $7}' | cut -d',' -f2 | cut -d'=' -f2 || echo "")
        if [ ! -z "$PIDS" ]; then
            echo "发现以下进程占用端口3000: $PIDS"
            for PID in $PIDS; do
                echo "正在强制终止进程 $PID..."
